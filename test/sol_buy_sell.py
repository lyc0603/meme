# I found a better way UPDATE COMING soon
...
wallet_address = "66ZC9U8y1uYaAxt4WFYVW11YZeZohvi8ev6wBHsAxykh"  #
seen_signatures = set()
    ...
    async def process_transactions(self):
        while True:
            ...
            try:
                ...
                account_signer = (
                    transaction.transaction.transaction.message.account_keys[0].pubkey
                )
                if account_signer == Pubkey.from_string(wallet_address):
                    print(
                        f"{getTimestamp()} -New update received: , https://solscan.io/tx/{signature}"
                    )
                    for ui_inner_instructions in instruction_list:
                        if ui_inner_instructions.instructions[
                            0
                        ].program_id == Pubkey.from_string(Pool_raydium):
                            print("Type 1 Txn")
                            first_info = ui_inner_instructions.instructions[1].parsed[
                                "info"
                            ]
                            second_info = ui_inner_instructions.instructions[2].parsed[
                                "info"
                            ]
                            first_token = transactionType(first_info["source"])
                            first_amount = int(
                                first_info["amount"]
                            ) / 10 ** getMintInfo(first_token)

                            if first_token == Pubkey.from_string(WRAPPED_SOL_MINT):
                                print(
                                    f"{style.GREEN}BUY {first_amount} SOL {style.RESET} -FOR  {int(second_info['amount']) / 10 ** getMintInfo(transactionType(second_info['destination']))} TokenBought= {transactionType(second_info['destination'])}  "
                                )

                            if transactionType(
                                second_info["destination"]
                            ) == Pubkey.from_string(raydium_V4):
                                print(
                        ...
                        if ui_inner_instructions.instructions[
                            ...
                            print("Type 2 Txn")
                            if (
                                ui_inner_instructions.index == 2
                                and "mint"
                                not in ui_inner_instructions.instructions[0].parsed[
                                    "info"
                                ]
                            ):
                                info = ui_inner_instructions.instructions[0].parsed[
                                    "info"
                                ]
                                # print(info)
                                ...
                                amount_sold = (
                                    int(info["amount"]) / 10**decimal_sell_token
                                )
                                second_info = ui_inner_instructions.instructions[
                                    1
                                ].parsed["info"]
                                sol_tokenAccount = second_info["destination"]
                                sol_decimal = getMintInfo(
                                    transactionType(sol_tokenAccount)
                                )
                                sol_amount = int(second_info["amount"]) / LAMPORTS
                                print(
                                    f"Sell {amount_sold} Token= {sell_token}  For {sol_amount} SOL "
                                )
