...
INITIAL_RPC_NODES = ["https://api.mainnet-beta.solana.com"]
...
def get_cluster_nodes(rpc_url):
    for attempt in range(RETRY_COUNT + 1):
        try:
            ...
            print(
                f"Error fetching getClusterNodes from {rpc_url} on attempt {attempt + 1}: {e}"
            )
    ...
    with ThreadPoolExecutor(max_workers=MAX_NODE_CHECK_WORKERS) as executor:
        future_to_node = {
            executor.submit(
                ...
                (
                    ...
                    else int(node["rpc"].split(":")[-1])
                ),
            ): node
            for node in nodes
            if (scan_null_rpc_nodes or node.get("rpc") is not None)
        }
        ...
        for future in tqdm(
            ...
            if result["is_active"]:
                ...
                active_nodes.append(
                    node_data
                )  # Сохраняем все данные ноды, включая остальные поля
    return active_nodes
...
if __name__ == "__main__":
    ...
    INITIAL_RPC_NODES.extend(
        loaded_rpc_nodes
    )  # Add loaded nodes to the initial RPC list
    ...
    with ThreadPoolExecutor(max_workers=MAX_RPC_FETCH_WORKERS) as executor:
        future_to_rpc = {
            executor.submit(get_cluster_nodes, rpc_url): rpc_url
            for rpc_url in INITIAL_RPC_NODES
        }

        for future in tqdm(
            as_completed(future_to_rpc),
            total=len(future_to_rpc),
            desc="Fetching nodes from initial RPCs",
            unit="RPC",
        ):
            nodes = future.result()
            if nodes:
    ...
    if new_nodes:
        ...
        active_nodes += check_all_nodes(
            new_nodes, scan_null_rpc_nodes=SCAN_NULL_RPC_NODES
        )
