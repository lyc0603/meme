"""This script analyzes trader exit probabilities based on lagged returns."""
...
df = pj_pft[pj_pft["meme_num"] <= 1000].copy()
df.drop_duplicates(subset=["trader_address", "meme"], keep="last", inplace=True)
df["date"] = pd.to_datetime(df["date"])
df = df.sort_values(["trader_address", "date"]).reset_index(drop=True)
df["project_idx"] = df.groupby("trader_address").cumcount()

# --- Exit labeling ---
cutoff_date = pd.Timestamp("2025-06-15")
last_trade_date = df.groupby("trader_address")["date"].idxmax()
df["exit"] = 0
exit_idx = df.loc[last_trade_date]
exit_idx = exit_idx[exit_idx["date"] < cutoff_date].index
df.loc[exit_idx, "exit"] = 1
...
df = parallel_apply(df.groupby("trader_address"))
...
# --- Render LaTeX Table ---
def render_multicolumn_latex_table(models, y_name="Exit$_{it}$", var_rename=None):
    ...
    tab.append("\\toprule")
    tab.append(" & \\multicolumn{4}{c}{$\\text{Exit}_{i,t}$} \\\\")
...
with open(TABLE_PATH / "reg_exit.tex", "w", encoding="utf-8") as f:
