...
wt_list = []
for chain in CHAINS:
    df = pd.read_csv(f"{PROCESSED_DATA_PATH}/wt_cm/comment_bots_{chain}.csv")
    df["first_comment_time"] = pd.to_datetime(
        df["first_comment_time"], format="mixed"
    ).dt.strftime("%Y-%m-%d %H:%M:%S%z")

    df = df.dropna().assign(
        category=chain,
        weight=meme_project_ratio[chain],
        first_comment_time=lambda x: pd.to_datetime(x["first_comment_time"]),
...
with open(PROCESSED_DATA_PATH / "wt_cm" / "delta_set_cm.pkl", "wb") as f:
...
cohorts_df.to_csv(PROCESSED_DATA_PATH / "reg_comment_did.csv", index=False)
...
latex_path = TABLE_PATH / "reg_comment_did.tex"
