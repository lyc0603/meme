...
def run_regression(
    ...
    X = pd.DataFrame(
        {
            f"creator": df["creator"],
            f"creator_x_{x_var}": df["creator"] * df[x_var],
            f"non_creator": 1 - df["creator"],
...
def main():
    ...
    for chain in ["pre_trump_raydium"]:
        reg_tab = pd.read_csv(f"{PROCESSED_DATA_PATH}/pft_{chain}.csv")
        ...
        with open(TABLE_PATH / f"reg_pft_{chain}.tex", "w", encoding="utf-8") as f:
            f.write(latex_str)
